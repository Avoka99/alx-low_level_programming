...
void is_abi(unsigned char *e_ident);
...
/**
 * is_osabi - Prints the OS/ABI of an ELF header.
 * @e_ident: A pointer to an array containing the ELF version.
 */
void is_osabi(unsigned char *e_ident)
{
...
/**
 * is_abi - Prints the ABI version of an ELF header.
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	rd = read(idx, header, sizeof(Elf64_Ehdr));
	if (rd == -1)
	{
		free(header);
		close_elf(idx);
